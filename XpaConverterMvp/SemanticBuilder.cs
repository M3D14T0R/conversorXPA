...
internal static class SemanticBuilder
{
    ...
    private static ExpressionSemantic BuildExpressionSemantic(TaskDef task)
    {
        ...
        var entries = task.Expressions
            .Select(e => new ExpressionEntrySemantic(
                e.Ordinal,
                e.Syntax,
                NormalizeExpressionLiteralSyntax(e.Syntax),
                e.Attribute,
                NormalizeExpressionSyntax(e.Syntax),
    ...
    private static string NormalizeExpressionLiteralSyntax(string syntax)
    {
        if (string.IsNullOrWhiteSpace(syntax))
            return "";

        var expr = RewriteLiteralPostfixTokens(syntax);
        expr = RewriteNormalizedFunctionCalls(expr);
        expr = RewriteMalformedNotQualifier(expr, "EmptyDataview");
        expr = RewriteMalformedNotQualifier(expr, "FileExist");
        expr = SimplifyDoubleNotCalls(expr);
        return expr;
    }
    ...
    private static string NormalizeExpressionSyntax(string syntax)
    {
        if (string.IsNullOrWhiteSpace(syntax))
            return "";
    ...
    private static string RewriteLiteralPostfixTokens(string expr)
    {
        ...
        while (i < expr.Length)
        {
            ...
            {
                var replacement = RewriteLiteralPostfixToken(token, literalValue);
    ...
    private static string? RewriteLiteralPostfixToken(string token, string literalValue)
    {
        if (string.Equals(token, "LOG", StringComparison.OrdinalIgnoreCase))
        {
            if (literalValue.Equals("TRUE", StringComparison.OrdinalIgnoreCase))
                return "true";
            if (literalValue.Equals("FALSE", StringComparison.OrdinalIgnoreCase))
                return "false";
            return null;
        }

        if (string.Equals(token, "PROG", StringComparison.OrdinalIgnoreCase))
        {
            var commaIndex = literalValue.IndexOf(',');
            if (commaIndex > 0 && int.TryParse(literalValue[..commaIndex], out var progId))
                return progId.ToString(CultureInfo.InvariantCulture);
            return null;
        }
        ...
        if (string.Equals(token, "EXP", StringComparison.OrdinalIgnoreCase) &&
            int.TryParse(literalValue, out var expId))
            return $"Exp_{expId}()";
    ...
    private static bool TryReadXpaLiteral(string text, int startIndex, out int endIndex, out string literalValue, out string literalCode)
    {
        ...
        endIndex = i;
        literalValue = text[(startIndex + 1)..i];
        literalCode = ToCSharpLiteral(literalValue);
        return true;
    }
    ...
    private static string RewriteMalformedNotQualifier(string expr, string functionName)
    {
        expr = RewriteMalformedFunctionQualifier(expr, "u.Not", functionName, $"u.{functionName}");
        expr = RewriteMalformedFunctionQualifier(expr, "u.Not", $"u.{functionName}", $"u.{functionName}");
        return expr;
    }

    private static string RewriteMalformedFunctionQualifier(string expr, string outerFunction, string innerFunction, string replacement)
    {
        var searchStart = 0;
        while (searchStart < expr.Length)
        {
            var outerIndex = expr.IndexOf(outerFunction, searchStart, StringComparison.OrdinalIgnoreCase);
            if (outerIndex < 0)
                break;

            var outerArgsStart = outerIndex + outerFunction.Length;
            while (outerArgsStart < expr.Length && char.IsWhiteSpace(expr[outerArgsStart]))
                outerArgsStart++;
            if (outerArgsStart >= expr.Length || expr[outerArgsStart] != '(')
            {
                searchStart = outerIndex + outerFunction.Length;
                continue;
            }

            var outerArgsEnd = FindMatchingParen(expr, outerArgsStart);
            if (outerArgsEnd <= outerArgsStart)
            {
                searchStart = outerIndex + outerFunction.Length;
                continue;
            }

            var outerArg = expr[(outerArgsStart + 1)..outerArgsEnd].Trim();
            if (!string.Equals(outerArg, innerFunction, StringComparison.OrdinalIgnoreCase))
            {
                searchStart = outerArgsEnd + 1;
                continue;
            }

            var invokeStart = outerArgsEnd + 1;
            while (invokeStart < expr.Length && char.IsWhiteSpace(expr[invokeStart]))
                invokeStart++;
            if (invokeStart >= expr.Length || expr[invokeStart] != '(')
            {
                searchStart = outerArgsEnd + 1;
                continue;
            }

            var invokeEnd = FindMatchingParen(expr, invokeStart);
            if (invokeEnd <= invokeStart)
            {
                searchStart = invokeStart + 1;
                continue;
            }

            var invocationArgs = expr[(invokeStart + 1)..invokeEnd];
            var rewritten = $"u.Not({replacement}({invocationArgs}))";
            expr = expr[..outerIndex] + rewritten + expr[(invokeEnd + 1)..];
            searchStart = outerIndex + rewritten.Length;
        }
    ...
    private static string RewriteNormalizedFunctionCalls(string expr)
    {
        expr = RewriteFunctionCalls(expr, name => string.Equals(name, "ExpCalc", StringComparison.OrdinalIgnoreCase), (name, args) =>
        {
            if (args.Count == 1 && Regex.IsMatch(args[0].Trim(), @"^Exp_\d+\(\)$", RegexOptions.IgnoreCase))
                return args[0].Trim();
            return null;
        });
        ...
        {
            return args.Count == 1 && int.TryParse(args[0].Trim(), out var level) ? $"u.Level({level})" : null;
        });
        expr = RewriteFunctionCalls(expr, name => string.Equals(name, "u.IniPut", StringComparison.OrdinalIgnoreCase), (name, args) =>
        {
            if (args.Count == 0 || !TryParseWholeCSharpStringLiteral(args[0].Trim(), out var keyLiteral))
                return null;
            var normalizedKey = keyLiteral.Replace("==", "=").Replace(" = ", "=");
            var rewrittenArgs = args.ToArray();
            rewrittenArgs[0] = ToCSharpLiteral(normalizedKey);
            return $"{name}({string.Join(", ", rewrittenArgs)})";
        });
        return expr;
    }

    private static string SimplifyDoubleNotCalls(string expr)
    {
        for (var i = 0; i < 8; i++)
        {
            var changed = false;
            expr = RewriteFunctionCalls(expr, name => string.Equals(name, "u.Not", StringComparison.OrdinalIgnoreCase), (name, args) =>
            {
                if (args.Count != 1)
                    return null;
                if (!TryParseFunctionCall(args[0].Trim(), out var innerName, out var innerArgs))
                    return null;
                if (!string.Equals(innerName, "u.Not", StringComparison.OrdinalIgnoreCase) || innerArgs.Count != 1)
                    return null;
                changed = true;
                return innerArgs[0].Trim();
            });
    ...
    private static string RewriteFunctionCalls(string expr, Func<string, bool> namePredicate, Func<string, IReadOnlyList<string>, string?> rewriter)
    {
        var sb = new StringBuilder(expr.Length);
        var i = 0;
        while (i < expr.Length)
        {
            if (TryReadQuotedSegmentEnd(expr, i, out var quotedEnd))
            {
                sb.Append(expr, i, quotedEnd - i + 1);
                i = quotedEnd + 1;
                continue;
            }
            ...
            var nameStart = i;
            var nameEnd = ReadIdentifierChainEnd(expr, i);
            var functionName = expr[nameStart..nameEnd];
            var cursor = nameEnd;
            while (cursor < expr.Length && char.IsWhiteSpace(expr[cursor]))
                cursor++;

            if (cursor >= expr.Length || expr[cursor] != '(' || !namePredicate(functionName))
            {
            ...
            var callEnd = FindMatchingParen(expr, cursor);
            if (callEnd <= cursor)
            {
                sb.Append(expr[nameStart]);
                i = nameStart + 1;
                continue;
            }

            var args = SplitTopLevelArguments(expr[(cursor + 1)..callEnd]);
            var rewritten = rewriter(functionName, args);
            if (!string.IsNullOrWhiteSpace(rewritten))
                sb.Append(rewritten);
            else
                sb.Append(expr, nameStart, callEnd - nameStart + 1);
            i = callEnd + 1;
        }
    ...
    private static bool TryParseFunctionCall(string expr, out string functionName, out IReadOnlyList<string> args)
    {
        functionName = "";
        ...
        var nameEnd = ReadIdentifierChainEnd(trimmed, 0);
        functionName = trimmed[..nameEnd];
        var cursor = nameEnd;
        ...
        var end = FindMatchingParen(trimmed, cursor);
        if (end != trimmed.Length - 1)
            return false;

        args = SplitTopLevelArguments(trimmed[(cursor + 1)..end]);
        return true;
    }
    ...
    private static IReadOnlyList<string> SplitTopLevelArguments(string text)
    {
        ...
        while (i < text.Length)
        {
            if (TryReadQuotedSegmentEnd(text, i, out var quotedEnd))
            {
                i = quotedEnd + 1;
                continue;
            }
    ...
    private static bool TryParseWholeCSharpStringLiteral(string expr, out string value)
    {
        value = "";
        if (string.IsNullOrWhiteSpace(expr))
            return false;
        ...
        {
            value = trimmed[2..^1].Replace("\"\"", "\"", StringComparison.Ordinal);
            return true;
        }
    ...
    private static bool TryReadQuotedSegmentEnd(string text, int startIndex, out int endIndex)
    {
        ...
        while (i < text.Length)
        {
            if (quote == '"' && verbatim && text[i] == '"' && i + 1 < text.Length && text[i + 1] == '"')
            {
                i += 2;
                continue;
            }
    ...
    private static int ReadIdentifierChainEnd(string text, int start)
    {
        var i = start;
        while (i < text.Length)
        {
    ...
    private static string? ResolveWholeExpressionSemanticKind(string syntax)
    {
        ...
        var normalized = NormalizeExpressionSyntax(syntax)
